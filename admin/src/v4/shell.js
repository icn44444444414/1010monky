...
function openSignOutModal() {
  showModal({
    title: 'Logga ut?',
    size: 'sm',
    body: '<p style="font-size:13px;color:var(--text-secondary);line-height:1.6;margin:0">Du behöver logga in igen för att komma åt panelen.</p>',
    actions: [
      { label: 'Avbryt', variant: 'ghost' },
      {
        ...
        variant: 'primary',
        action: () => {
          showToast('Loggar ut…', { variant: 'success' });
          setTimeout(() => { window.location.href = '/admin/logout'; }, 400);
        }
      }
    ]
  });
...
const USER_MENU = [
  { label: 'Tangentbordsgenvägar', action: openShortcutsModal },
  '-',
  { label: 'Logga ut',             action: openSignOutModal }
];
